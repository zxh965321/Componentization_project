...
//<editor-fold desc="DarkMode">
         ...
         */
        fun setFontBlack(activity: Activity?, isDark: Boolean) {
            activity?.let {
                if (isXiaomi() && Build.VERSION.SDK_INT < Build.VERSION_CODES.M) { //对于小米手机6.0以上,应该优先调用小米的方法,而不是M上的,应该改ROM了
                    darkModeForMIUI6(activity.window, isDark)
                } else if (isMeizu()) {
                    darkModeForFlyme4(activity.window, isDark)
